...
### 1. Gỡ service role key khỏi frontend → Edge Function — ✅ XONG (code), ⏳ chờ deploy

**Đã làm:** Tạo Edge Function duy nhất `supabase/functions/admin/index.ts` giữ
service key ở server; thêm `src/lib/adminApi.ts`; thay toàn bộ `supabaseAdmin`
trong frontend (LoginScreen, AddStaffForm, StaffProfile, AppContext) bằng lời
gọi Edge Function hoặc client thường (RLS admin). Đã **gỡ `supabaseAdmin` +
service key khỏi `src/lib/supabase.ts`**. Không còn `SERVICE_ROLE` nào trong `src/`.

**Việc của bạn (1 lần, ~5 phút):** deploy function theo
[`docs/EDGE_FUNCTION_DEPLOY.md`](./EDGE_FUNCTION_DEPLOY.md):
- [ ] `supabase functions deploy admin --no-verify-jwt`
- [ ] Xóa `VITE_SUPABASE_SERVICE_ROLE_KEY` khỏi Vercel env + redeploy
- [ ] Chạy checklist kiểm thử trong tài liệu deploy

<details><summary>Chi tiết kỹ thuật cũ (đã hoàn thành)</summary>
...
### 2. Random mật khẩu tạm thay cho hardcode (bạn đồng ý tạm bỏ qua)
- [ ] `src/components/hr/AddStaffForm.tsx` — vẫn dùng `tempPassword = 'fest1234'`.
      Rủi ro nhỏ ở quy mô nhỏ; chỉ cần dặn nhân viên đổi mật khẩu ngay lần đầu.
      Khi muốn: đổi sang random + hiện 1 lần cho admin.
